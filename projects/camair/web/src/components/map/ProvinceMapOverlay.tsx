...
function ProvinceGeoJSON({
  ...
  useEffect(() => {
    styleProvinceRef.current = styleProvince;
  }, [styleProvince]);

  const onEachProvince = useCallback((feature: GeoJsonFeature, layer: L.Layer) => {
    const name = feature.properties.adm1_name;

    layer.bindTooltip("", { sticky: true, className: "custom-tooltip" });
    layer.bindPopup("", { closeButton: false, offset: [0, -10] });

    layer.on({
      click: () => {
        onSelectProvinceRef.current(name);
      },
      mouseover: (e: LeafletEvent) => {
        const l = e.target as L.Path;
        if (l.options.interactive === false) return;
        l.setStyle({
          weight: 3,
          color: "#334155",
      ...
      mouseout: (e: LeafletEvent) => {
        ...
        if (l.feature && l.options.interactive !== false) {
          l.setStyle(styleProvinceRef.current(l.feature));
        }
      },
